...
const itemStyles = tv({
  extend: focusRing,
  base: "relative grid grid-cols-[auto_1fr_auto_auto] gap-3 items-center cursor-default select-none py-2 px-3 text-sm text-gray-900 dark:text-zinc-200 border-y dark:border-y-zinc-700 border-transparent first:border-t-0 last:border-b-0 first:rounded-t-md last:rounded-b-md -mb-px last:mb-0 -outline-offset-2 min-w-64",
  variants: {
    isSelected: {
...
export const GridListItem = React.memo(function GridListItem({
  ...
  return (
    <AriaGridListItem textValue={textValue} {...props} className={itemStyles}>
      {(renderProps) => {
        // Handle the case where children might be a function (React Aria pattern)
        const childrenContent =
          typeof children === "function" ? children(renderProps) : children;
        const childrenArray = React.Children.toArray(childrenContent);
        const filename = childrenArray[0];
        const additionalElements = childrenArray.slice(1);
        ...
        return (
          <>
            ...
            <div className="flex gap-1">{additionalElements}</div>
            {onRemove && (
              <Button
